...
def probe_candidates(exclude_paths=()):
    ...
    carrying its raw v4l2-ctl description under "description". New in
    v0.2.1.
    """
    devs = list_video_devices()
    candidates = [p for p in find_brio_devices() if p not in exclude_paths]
    results = []
    for path in candidates:
        info = probe_device(path)
        if not info:
        ...
        if known is not None:
            ...
            info["classified_by"] = "saturation_probe"
        info["description"] = devs.get(path)
        results.append(info)
    return results
